import Container from "@/utils/Container";

const AboutCard = () => {
  return (
    <Container className="py-10">
      <div className="relative h-[583px]">
        <div
          className="relative w-[70%] h-[70%] bg-no-repeat bg-cover bg-center z-10"
          style={{
            backgroundImage: `url(https://thedailynotes.com/wp-content/uploads/2024/01/Investing-In-Real-Estate.jpg)`,
          }}
        >
          <div className="absolute inset-0 bg-gradient-to-b from-transparent via-black/70 to-black/90"></div>
        </div>
        <div className="absolute inset-0 w-[50%] h-[200px]  bg-accent top-1/2 left-1/2 px-4 z-50 ">
        <div className="flex items-center gap-2 mt-4">
            <div className="w-8 border h-[1px] border-secondary"></div>
            <p className="text-secondary font-semibold">Our Story</p>
          </div>
          <h1
            className="text-title text-[36px] text-white font-bold"
            style={{ fontFamily: "Montserrat" }}
          >
            Efficient and Transparent Home Buying Solutions
          </h1>
          <p
            className="text-muted text-[16px] leading-[24px] font-medium"
            style={{ fontFamily: "Poppins" }}
          >
            In the symphony of bustling markets and serene landscapes,
            Heritage-Nest was born—a vision to
          </p>
        </div>
      </div>

      <div className="relative h-[583px]">
        <div className="absolute inset-0 w-[50%]  bg-accent top-1/2 right-1/2 px-4 z-50 ">
        <div className="flex items-center gap-2 mt-4">
            <div className="w-8 border h-[1px] border-secondary"></div>
            <p className="text-secondary font-semibold">Mission Statement</p>
          </div>
          <h1
            className="text-title text-[36px] text-white font-bold mb-4"
            style={{ fontFamily: "Montserrat" }}
          >
            Efficient and Transparent Home Buying Solutions
          </h1>
          <p
            className="text-muted text-[16px] leading-[24px] font-medium"
            style={{ fontFamily: "Poppins" }}
          >
           To bridge the miles with smiles, Heritage-Nest pledges to be the golden thread connecting NRIs to their homeland, through trust, transparency, and tailored real estate solutions.
          </p>
        </div>
        <div
          className="absolute right-0 w-[70%] h-[70%] bg-no-repeat bg-cover bg-center z-10"
          style={{
            backgroundImage: `url(https://thedailynotes.com/wp-content/uploads/2024/01/Investing-In-Real-Estate.jpg)`,
          }}
        >
          <div className="absolute inset-0 bg-gradient-to-b from-transparent via-black/70 to-black/90"></div>
        </div>
      </div>
    </Container>
  );
};

export default AboutCard;
